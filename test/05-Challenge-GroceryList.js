...
describe('05 - Challenge - Grocery List', () => {
  ...
  describe('Task #1 - display a list of groceries', () => {
    ...
    it('There should be an unordered list of groceries', () => {
      const groceryListItems = React.addons.TestUtils.scryRenderedDOMComponentsWithTag(component, 'li')
      assert.equal(groceryListItems.length, 1, 'There should be exactly one element on the groceries list')

      const groceryItem = _.first(groceryListItems)
  ...
  describe('Task #2 - add a new product to list', () => {
    ...
    it('Should render required tags like additional button and input', () => {
      try { React.addons.TestUtils.findRenderedDOMComponentWithClass(component, 'new-item') } catch (err) {
        throw new Error("I can't find new item input")
      }
      try { React.addons.TestUtils.findRenderedDOMComponentWithClass(component, 'add-product') } catch (err) {
        throw new Error("I can't find 'Add new Product' button")
      }
    })

    it('Should be possible to add a new product to list', () => {
      const newProductInput = React.addons.TestUtils.findRenderedDOMComponentWithClass(component, 'new-item')
      const newProductAddButton = React.addons.TestUtils.findRenderedDOMComponentWithClass(component, 'add-product')
      ...
      const groceryItem = _.last(groceryListItems)
  ...
  describe('Task #3 - clearing groceries list', () => {
    ...
    it('Should render required tags', () => {
      try { React.addons.TestUtils.findRenderedDOMComponentWithClass(component, 'clear-list') } catch (err) {
        throw new Error("I can't find 'Clear the List' button")
      }
    })

    it('Should be possible to remove all list items', () => {
      const clearListButton = React.addons.TestUtils.findRenderedDOMComponentWithClass(component, 'clear-list')
